...
const ExportShare = {
  gerarRelatorioWhatsApp(dadosBolao, apuracao, financeiro, cicloAtual) {
    ...
    let msg = `🍀 *${nomeBolao.toUpperCase()} — ${cicloAtual.nome || 'CICLO ATUAL'}* 🍀\n`;
    msg += `📊 *Boletim de Apuração — ${dataHora}*\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━━\n\n`;

    if (concursos.length === 0) {
      msg += `📌 *Ciclo Iniciando:* Concurso ${cicloAtual.concursoInicial || '3058'}\n`;
      ...
      msg += `🎯 *Ciclo Atual:* Concursos ${primeiro} a ${ultimo} (${concursos.length} sorteios acumulados)\n\n`;

      msg += `🎲 *DEZENAS SORTEADAS NESTE CICLO:*\n`;
    ...
    if (ganhadoresQuadraPrimeiroSorteio && ganhadoresQuadraPrimeiroSorteio.length > 0) {
      ...
      ganhadoresQuadraPrimeiroSorteio.forEach(g => {
        msg += `🎉 *${g.nome}* acertou ${g.acertosNoPrimeiroSorteio} dezenas na abertura do ciclo!\n`;
      });
  ...
  <div class="page-container">
    ...
    <header class="compact-header">
      <div class="row-top">
        ...
        <div class="meta-wrap">
          <span><strong>Ciclo:</strong> ${cicloAtual.nome || 'Ciclo 1'} (Conc. ${concursos.length > 0 ? concursos[0].numero : cicloAtual.concursoInicial} a ${concursos.length > 0 ? concursos[concursos.length - 1].numero : '...'})</span>
          <span><strong>Emissão:</strong> ${dataHora}</span>
          <span>${cicloAtual.status === 'finalizado' ? '🏆 Encerrado' : '⏳ Em Andamento'}</span>
        </div>
      </div>
